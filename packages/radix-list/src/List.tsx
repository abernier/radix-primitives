...
const [ListProvider, useListContext] =
  createListContext<ListContextValue>(LIST_NAME);
...
const List = React.forwardRef<ListElement, ScopedProps<ListProps>>(
  (props, forwardedRef) => {
    const {
      __scopeList,
...
const ListItem = React.forwardRef<ListItemElement, ScopedProps<ListItemProps>>(
  (props, forwardedRef) => {
    ...
    const { selectedKeys, onSelect, orientation } = useListContext(
      LIST_NAME,
      __scopeList,
    );
